...
impl<'a> GlobalMap<'a> {
    pub fn combined<'b>(private: &'b mut GlobalMapPrivateState, shared: specs::shred::FetchMut<'b, GlobalMapSharedState>) -> GlobalMap<'b> {
        GlobalMap::<'b> {
            private,
            shared
        }
    ...
    pub fn update(&mut self) {
        ...
        if self.private.loaded.0 > x - Self::LOAD_AHEAD {
            let (low, high) = (self.private.loaded.0 - 1 - Self::LOAD_STEP, self.private.loaded.0 - 1);
        ...
        if self.private.loaded.1 <  x + self.private.view_width + Self::LOAD_AHEAD {
            let (low, high) = (self.private.loaded.1 + 1, self.private.loaded.1 + 1 + Self::LOAD_STEP);
            crate::net::request_map_read(low, high);
    ...
    fn draw_villages(&mut self, window: &mut Window, sprites: &mut Sprites) -> Result<()> {
        ...
        for vil in &self.private.villages {
            ...
            let sprite_area = Rectangle::new(
                (x as f32 * self.shared.scaling, y as f32 * self.shared.scaling),
        ...
        for s in &self.private.skeleton.streams {
            for (x,y) in s {
                let area = Rectangle::new(
                    (x * self.shared.scaling - pt/2.0, y * self.shared.scaling - pt/2.0),
                    (pt, pt),
                );
                window.draw_ex(
                    &area, 
