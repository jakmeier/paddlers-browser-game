...
impl Town {
    ...
    pub fn closest_walkable_tile_in_range(&self, start: TileIndex, destination: TileIndex, radius: f32) -> Option<TileIndex> {
        let valid = self.tiles_in_rectified_circle(destination, radius);

        let successors = |v: &TileIndex| self.successors(*v);
        let success = |v: &TileIndex| valid.contains(v);
        let heuristic = |v: &TileIndex| (absdiff(v.0, destination.0) + absdiff(v.1, destination.1)) as u32;
